...
ytmLoss    = 0       #0: mimimize squared price errors: 1: minimize squared ytm errors
  weightLoss = 1       #eg. 1/sqrt(mat) as weights in loss fn (only if ytmLoss==0)
#------------------------------------------------------------------------------
...
println("\nImplied spot rates (test): ",round(100*shx[[1:3;end-3:end]],2))
figure()
  plot(tmFig,shx)
#----------------------------------------------------------------------------
...
if ytmLoss == 1
  ...
  if weightLoss == 1
    NSXbR = BondNSxEstPs(parX0,pdat,tm,c,log(1+y[1]),0,1./sqrt(tm))
  else
...
fhx = exp(fhx) - 1
println("\nImplied spot rates (NSXbR): ",round(100*shx[[1:3;end-3:end]],2))
