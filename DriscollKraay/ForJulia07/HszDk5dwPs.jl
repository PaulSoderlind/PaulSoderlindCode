function HszDk5dwPs(y,x,z,yhatQ=false,m=0,ScaleByNtQ=2,vvzx=[],wM=[])
...
#------------------------------------------------------------------------------
  ...
  for t = 1:T                             #loop over time
    ...
    if sum(vv) > 0                       #don't accumulate [] to xx and xy (generates [])
  ...
  for t = 1:T                           #loop over time
    ...
    if sum(vv) > 0                       #don't accumulate [] to omega0DK
